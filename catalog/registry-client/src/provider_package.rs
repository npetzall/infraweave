...
mod tests {
    ...
    #[test]
    fn shasum_from_file_rejects_malformed_detached_signature() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(dir.path().join("foo.zip"), b"z").unwrap();
        std::fs::write(dir.path().join("SHA256SUMS"), b"deadbeef *foo.zip\n").unwrap();
        std::fs::write(dir.path().join("SHA256SUMS.sig"), b"not-openpgp-signature").unwrap();
        let keyring_path = dir.path().join("kr.asc");
        std::fs::write(
            &keyring_path,
        ...
        let pkg = ProviderPackage {
            ...
            shasums: FileArtifact {
                filename: "SHA256SUMS".to_string(),
                path: dir.path().join("SHA256SUMS"),
            },
            signature: FileArtifact {
                filename: "SHA256SUMS.sig".to_string(),
                path: dir.path().join("SHA256SUMS.sig"),
            },
            keyring: FileArtifact {
                filename: "kr.asc".to_string(),
                path: keyring_path,
            },
        };
        assert!(matches!(
            pkg.shasum_from_file(),
            Err(ProviderRegistryError::ParseDetachedSignature { .. })
        ));
    }
}
